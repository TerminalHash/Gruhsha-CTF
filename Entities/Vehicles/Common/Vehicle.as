...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (isServer() && cmd == this.getCommandID("load_ammo"))
	{
		...
		if (eligible_ammo_names[0] == "mat_stone")
		{
			...
			stone.Init();
			if (getRules().get_s32("personalstone_" + caller.getTeamNum()) >= 250)
			{

				stone.server_SetQuantity(250);
				getRules().add_s32("personalstone_" + caller.getTeamNum(), -250);
				getRules().Sync("personalstone_" + caller.getTeamNum(), true);
			}
			else
			{
				stone.server_SetQuantity(getRules().get_s32("personalstone_" + caller.getTeamNum()));
				getRules().add_s32("personalstone_" + caller.getTeamNum(), -(getRules().get_s32("personalstone_" + caller.getTeamNum())));
				getRules().Sync("personalstone_" + caller.getTeamNum(), true);
			}
