...
void onInit(CBlob@ this)
{
	...
	if (isServer())
	{
		...
		if (map.getSector("capture zone "+this.getNetworkID()) is null)
		{
			...
			Vec2f corner(this.getRadius() + 16.0f, this.getRadius() + 16.0f);
			map.server_AddMovingSector(corner*-2, corner*2, "capture zone "+this.getNetworkID(), this.getNetworkID());
		}
	}
}
...
void onChangeTeam(CBlob@ this, const int oldTeam)
{
	...
	if (this.getTeamNum() == 0 && oldTeam != 0) {
		getRules().set_bool("cp_controlled_blue", true);

		// if point is being controlled by red - reset it
		getRules().set_bool("cp_controlled_red", false);
	} else if (this.getTeamNum() == 1 && oldTeam != 1) {
		getRules().set_bool("cp_controlled_red", true);

		// if point is being controlled by blue - reset it
		getRules().set_bool("cp_controlled_blue", false);
	}
}
