...
void onTick(CBlob@ this)
{
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		if (getNet().isClient())
		{
			if (knight.swordTimer == KnightVars::slash_charge_level2 && knight.state == 4)
			{
				Sound::Play("ArgLong.ogg", pos, myplayer ? 1.4f : 0.8f, 0.95f);
			}
		}

		bool strong = (knight.swordTimer > KnightVars::slash_charge_level2);
		moveVars.jumpFactor *= (strong ? 0.8f : 1.1f);
		moveVars.walkFactor *= (strong ? 0.7f : 1.1f);
		knight.shieldTimer = 0;
		...
		if (knight.state == KnightStates::sword_drawn && knight.swordTimer >= KnightVars::slash_charge_level2 && pressed_a1 && getGameTime() % 10 == 0)
		{
			Sound::Play("/SwordSlash", this.getPosition(), 0.9f, 0.5f + (XORRandom(6)/9));
		}
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			if (delta == 2)
			{
				...
				Sound::Play("/SwordSlash", this.getPosition(), 1.4f, 0.3f);
			}
