...
void Take(CBlob@ this, CBlob@ blob)
{
    ...
    if (blobName == "mat_bombs" || (blobName == "satchel" && !blob.hasTag("exploding")) || blobName == "mat_waterbombs" || blobName == "mat_stickybombs" || blobName == "mat_icebombs" || blobName == "mat_jarate")
    {
	...
	if (blobName == "drill") {
		...
		if (p !is null) {
			...
			if (blob.canBePickedUp(this) && blob.canBePutInInventory(this) && getRules().get_string(username + "pickdrill_builder") != "no") {
				if (this.server_PutInInventory(blob)) {
					return;
				}
			}
