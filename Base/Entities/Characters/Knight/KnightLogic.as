				...
				for (int j = 0; j < rayInfos.size(); j++)
				{
					...
					if (rayb.getName() == "log")
					{
						if (!dontHitMoreLogs)
						{
							...
							{
								getRules().add_s32("personalwood_" + this.getPlayer().getTeamNum(), quantity);
								getRules().Sync("personalwood_" + this.getPlayer().getTeamNum(), true);
							}
						...
						if (dif < 20.0f)
						{
							...
							if (canhit)
							{
								...
								else if (dirt_stone)
								{
									...
									if (isServer() && this.getPlayer() !is null)
									{
										getRules().add_s32("personalstone_" + this.getPlayer().getTeamNum(), quantity);
										getRules().Sync("personalstone_" + this.getPlayer().getTeamNum(), true);
									}
