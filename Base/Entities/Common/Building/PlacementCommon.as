...
shared class BlockCursor
{
	...
	BlockCursor()
	{
		blobActive = blockActive = buildableAtPos = rayBlocked = hasReqs = supported = buildable = buildable_alt = cursorClose = sameTileOnBack = false;
	}
};
...
bool isBuildableAtPosAlt(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile, string clientsideblob="null")
{
	f32 radius = 0.0f;
	...
	if (blob is null || !blob.hasTag("ignore blocking actors"))
	{
		...
		if(blobAtPos !is null && blob is null && (isDoor || isPlatform) && clientsideblob != "null")
		{
			if(blobAtPos.getConfig() == clientsideblob && 
		...
		if(blobAtPos !is null && blob is null && isPlatform && clientsideblob != "null")
		{
			if(blobAtPos.getConfig() == clientsideblob && 
				blobAtPos.getHealth() != blobAtPos.getInitialHealth()) 
			{	
		...
		const string name = blob !is null ? blob.getName() : clientsideblob;
		CBlob@[] blobsInRadius;
		if (map.getBlobsInRadius(middle, buildSolid ? map.tilesize : 0.0f, @blobsInRadius))
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				{
					if (blob !is null || buildSolid  || clientsideblob != "null")
					{
						...
						bool ladder_build_shop = false;

						if (b.getName() == "quarters" ||
							b.getName() == "storage" ||
							b.getName() == "knightshop" ||
							b.getName() == "buildershop" ||
							b.getName() == "archershop" ||
							b.getName() == "vehicleshop" ||
							b.getName() == "boatshop" ||
							b.getName() == "tunnel" ||
							b.getName() == "outpost" ||
							b.getName() == "building")
						{
							if (isLadder) ladder_build_shop = true;
						}
...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile, string clientsideblob="null")
{
	f32 radius = 0.0f;
	CMap@ map = this.getMap();
	CRules@ rules = getRules();
...
void SetupBuildDelay(CBlob@ this)
{
	...
	this.set_u32("build delay", 6);  // move this to builder init
