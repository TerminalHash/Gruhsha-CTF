...
void onRender(CSprite@ this)
{
	...
	if (mouseOnBlob)
	{
		...
		const f32 initialHealth = blob.get_s16("fuel_level");
		if (initialHealth > 0.0f)
		{
			const f32 perc = initialHealth / 500;
			if (perc >= 0.0f)
			{
				GUI::DrawRectangle(Vec2f(pos2d.x - dim.x - 2, pos2d.y + y - 2), Vec2f(pos2d.x + dim.x + 2, pos2d.y + y + dim.y + 2));
				GUI::DrawRectangle(Vec2f(pos2d.x - dim.x + 2, pos2d.y + y + 2), Vec2f(pos2d.x - dim.x + perc * 2.0f * dim.x - 2, pos2d.y + y + dim.y - 2), SColor(0xffe9b666));
			}
		}
	}
}
