...
void onInit(CBlob@ this)
{
	...
	this.set_string("custom_explosion_sound", "handgrenade_blast2.ogg");
	...
	CSpriteLayer@ fuse = this.getSprite().addSpriteLayer("fuse", "Hazelnut.png", 16, 16, 0, 0);
...
void SpawnShells(CBlob@ this)
{
	if(!isServer()) return;
	int shell_amount = 5;

	f32 flip_factor = this.isFacingLeft()?-1:1;

	for (int idx = 0; idx < shell_amount; ++idx)
	{
		CBlob@ hazelnutshell = server_CreateBlob("hazelnutshell",this.getTeamNum(),this.getPosition()+Vec2f((10 + 5) * flip_factor, -40));
