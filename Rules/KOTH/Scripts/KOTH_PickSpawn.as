...
void BuildRespawnMenu(CRules@ this, CPlayer@ player, CBlob@[] respawns)
{
	...
	if (menu !is null)
	{
		...
		for (uint i = 0; i < respawns.length; i++)
		{
			...
			CGridButton@ button = menu.AddButton("$" + respawn.getName() + "$", msg, "KOTH_PickSpawn.as", "Callback_PickSpawn", Vec2f(BUTTON_SIZE, BUTTON_SIZE), params);
			if (button !is null)
			{
...
void onTick(CRules@ this)
{
	...
	if (SHOW_MENU)
	{
		const string propname = "koth spawn time " + player.getUsername();
