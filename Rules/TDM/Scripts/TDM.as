	...
	void GiveSpawnResources(CBlob@ blob, CPlayer@ player)
	{
		...
		if (blob.getName() == "builder")
		{
			...
			if (!found)
			{
				...
				if (mat1 !is null)
				{
					if (!blob.server_PutInInventory(mat1))
					{
						mat1.setPosition(blob.getPosition());
					}
				}

				CBlob@ mat2 = server_CreateBlob("drill");
				if (mat2 !is null)
				{
					if (!blob.server_PutInInventory(mat2))
					{
						mat2.setPosition(blob.getPosition());
					}
				}
			}
		}
