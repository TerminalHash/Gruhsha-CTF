...
string streak_sound = soundsdir + "Sounds/Killstreaks/";
	...
	void Render()
	{
		...
		for (uint message_step = 0; message_step < count; ++message_step)
		{
			...
			switch (message.hitter)
			{
				...
				case GruhshaHitters::flail:				hitterIcon = "$killfeed_flail$"; break;
				case GruhshaHitters::knife:				hitterIcon = "$killfeed_knife$"; break;
...
void onInit(CRules@ this)
{
	...
	AddIconToken("$killfeed_flail$", "GUI/KillfeedIcons.png", Vec2f(32, 16), 31);
	AddIconToken("$killfeed_knife$", "GUI/KillfeedIcons.png", Vec2f(32, 16), 32);
}
	...
	if(isServer())
	{
		...
		{ 
			...
			if (player !is null && player.exists("killstreak"))
			{
				...
				if (getGameTime() - player.get_u32("kill time") > (6 * getTicksASecond()) && player.get_u8("killstreak") > 3)
				{
					string multiKill;
...
void onCommand(CRules@ this, u8 cmd, CBitStream @params)
{
	if (isClient())
	{
		if (cmd == this.getCommandID("killstreak message"))
		{
			...
			switch (kill_count)
			{
				case 4: multiKill = "a Killing spree";
					break;
			...
			// Play sound, if player has killstreak
			if (kill_count == 4)
			{
				Sound::Play(streak_sound + "killingspree.ogg");
			}
			else if (kill_count == 5)
			{
				Sound::Play(streak_sound + "rampage.ogg");
			}
			else if (kill_count == 6)
			{
				Sound::Play(streak_sound + "unstoppable.ogg");
			}
			else if (kill_count == 7)
			{
				Sound::Play(streak_sound + "monsterkill.ogg");
			}
			else if (kill_count == 8)
			{
				Sound::Play(streak_sound + "megakill.ogg");
			}
			else if (kill_count >= 8)
			{
				Sound::Play(streak_sound + "godlike.ogg");
			}
		}
